...
def update_CLGraph(y,templ, dt, SNtype):
    ...
    if y != 'All Colors':
        ...
        fig.update_layout(title= y+" Supernovae "+SNtype+ " Light Curves", xaxis3=dict(title="Days since first detection"), yaxis=dict(title= filt[0] + ' Absolute Mag'), yaxis2=dict(title=filt[1] + ' Absolute Mag'), yaxis3=dict(title= y), legend_title= "Supernovae" + " Type "+ SNtype+"'s", height=625,template= templ, showlegend=True, updatemenus=[
            dict(
                    type="buttons", showactive=False, xanchor="left", yanchor="top", x=0.0, y=1.08, buttons=list(
                        [   dict(
                                label="Colors Only",
                                method="update",
                                args=[{"visible": [True, True, True]},{'title': y +" Supernovae "+ SNtype+" Light Curves", 'yaxis.title': filt[0] + ' Absolute Mag', 'yaxis2.title': filt[1] + ' Absolute Mag', 'yaxis3.title': y, "yaxis.visible": True, "yaxis2.visible": True,"yaxis.domain":[.66667, 1], "yaxis2.domain": [.33334,.66667],"yaxis3.domain": [0,.33334]}],
                                args2= [{"visible": [False, False, True]},{'title': y+" Supernovae "+SNtype+ " Color Light Curves", 'yaxis3.title': y, "yaxis.visible": False, "yaxis2.visible": False,  "yaxis3.domain":[0,1]}]
